...
void record_noise(int x, int y, int z, float noise, struct noisemap *map)
{
	map->noise[x + y * map->width + z * map->width * map->height] = noise < 0 ? 0 : (unsigned char)(noise * 255.99);
}

struct noisemap *perlin_create_noise_map(int seed, int width, int height, int no_z, int noiter, float scale, float centerweight)
{
	int i;

	struct noisemap *map = s_malloc(sizeof(struct noisemap), NULL);
	map->height = height;
	map->width = width;
	map->depth = no_z;
	map->noise = s_malloc(width * height * no_z * sizeof(float), NULL);

	perlin_noise_iter(seed, width, height, no_z, noiter, scale, centerweight, map, record_noise);

	return map;
}

void perlin_destroy_noise_map(struct noisemap *map)
{
	s_free(map->noise, NULL);
	s_free(map, NULL);
}
	...
	for(z = 0; z < no_z; z++)
	{
		...
		for(y = 0; y < height; y++)
		{
			for(x = 0; x < width; x++)
			{
				...
				for(i = 0; i < noiter; i++)
				{
					n += (((noise(x * scale * pow2 + xoff[i], y * scale * pow2 + yoff[i], z) + 1) * (1 - c)) / (2.0 * pow2));
					pow2 *= 2;
				}
				n /= divisor;
				func(x, y, z, n, data);
